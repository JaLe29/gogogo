...
type BaseField = {
    rules?: Rule[];
    footer?: AnyReactElement;
};

type FieldString = {
    label: AnyReactElement;
    key: string;
    required?: boolean;
    placeholder?: string;
...
type FieldCheckBox = {
    ...
    required?: boolean;
    type: 'checkbox';
    node: AnyReactElement;
} & BaseField;

export type Field = FieldDayPicker | FieldString | FieldSelect | FieldCheckBox | FieldCheckBoxGroup;

const isFieldSelect = (field: Field): field is FieldSelect => field.type === 'select';
const isFieldCheckBox = (field: Field): field is FieldCheckBox => field.type === 'checkbox';
...
const SimpleForm: React.FC<Props> = ({
    ...
    const resolveField = (field: Field): AnyReactElement => {
        ...
        if (isFieldSelect(field)) {
                    ...
                    <Select
                        ...
                        filterOption={(
                            ...
                            option?: {
                                ...
                                options: { label: string; value: string; children: AnyReactElement }[];
                            },
                        ) => {
                            if (option?.options) {
                                return false;
            ...
            return (
                <Select
                    ...
                    filterOption={(
                        input: string,
                        option?: { label: string; value: string; children: AnyReactElement },
                    ) => {
        ...
        if (field.type === 'day-picker') {
            const disabledDate = (d: any): boolean => {
                if (field.minDate && field.maxDate) {
                    return d.isBefore(field.minDate) || d.isAfter(field.maxDate);
                }

                if (field.minDate) {
                    return d.isBefore(field.minDate);
                }

                if (field.maxDate) {
                    return d.isAfter(field.maxDate);
                }
